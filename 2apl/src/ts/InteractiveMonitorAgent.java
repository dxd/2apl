...
public class InteractiveMonitorAgent extends StationaryAgent 
	...
	public void run() { 
		...
		try{
			...
			// NOTE: this reaction listens for any kind of change in the status 
			// of an agent.	The same idea can be applied to either hosts or
			// agents.
			...
			slts.addReaction(
				new LimeSystemReaction[]{lr1, lr2, lr3, lr4, lr5, lr6, lr7});
		} catch(LimeException le){ le.printStackTrace(); }
		while(true) { 
			c.performQueuedOp(); 
